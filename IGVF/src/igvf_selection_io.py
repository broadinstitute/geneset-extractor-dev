...
def igvf_root() -> Path:
    return repo_root() / "geneset-extractor-dev" / "IGVF"


def config_root() -> Path:
    return igvf_root() / "config"


def default_out_root() -> Path:
    return Path.cwd() / "igvf_all_models"
...
def default_analysis_set_list_path() -> Path:
    return config_root() / "analysis_set_list.tsv"
