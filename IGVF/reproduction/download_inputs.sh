...
inputs_root=${IGVF_INPUTS_ROOT:?Set IGVF_INPUTS_ROOT to the directory for released IGVF inputs.}
while IFS=$'\t' read -r analysis_set_id dataset_id file_relpath _; do
  ...
  if [[ ! -f "${destination}" ]]; then
    curl --fail --location --silent --show-error \
      "https://api.data.igvf.org/tabular-files/${dataset_id}/@@download/${filename}" \
      --output "${destination}"
  fi
