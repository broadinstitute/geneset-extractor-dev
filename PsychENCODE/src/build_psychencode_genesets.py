...
from psychencode_selection_io import (
...
SUPPORTED_FAMILIES = {"released_dex", "released_modules"}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--dex_csv", help="Released DER-13 Disorder DEX genes CSV (HZ1).")
    parser.add_argument("--modules_csv", help="Released DER-16 gene co-expression modules CSV (HZ2).")
...
def main() -> int:
    ...
    input_by_model = {
        "HZ1": require_existing_file(args.dex_csv, "disorder DEX CSV") if "HZ1" in selected_models else None,
        "HZ2": require_existing_file(args.modules_csv, "gene modules CSV") if "HZ2" in selected_models else None,
    }
    ...
    for model_id in selected_models:
        ...
        if model_family not in SUPPORTED_FAMILIES:
            raise SystemExit(f"Unsupported PsychENCODE model family for {model_id}: {model_family}")
        input_csv = input_by_model.get(model_id)
        if input_csv is None:
            raise SystemExit(f"Missing released input CSV for model {model_id}")
        run_command(
            [
                str(Path(args.python_bin).resolve()),
                str(src_root / "run_psychencode_hz_model.py"),
                "--model_id",
                model_id,
                "--run_root",
