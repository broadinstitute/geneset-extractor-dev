...
def source_script() -> Path:
    return repo_root() / "notebooks_adapted" / "build_hubmap_asctb_augmented_gmt.py"


def load_source_module(script_path: Path):
    spec = importlib.util.spec_from_file_location("hubmap_asctb_augmented_source", script_path)
