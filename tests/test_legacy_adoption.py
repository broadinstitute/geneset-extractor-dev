...
class LegacyAdoptionTest(unittest.TestCase):
    ...
    def test_adoption_prompt_is_pattern_aware_and_enforces_repository_boundary(self) -> None:
        ...
        self.assertIn("## Mandatory completion gate", prompt)
    ...
    def test_allowlist_preflight_requires_source_rules_and_never_stages_generated_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            cache = library / "src/__pycache__"
            cache.mkdir()
            (cache / "build.cpython-314.pyc").write_bytes(b"generated bytecode")
            ...
            self.assertNotIn("Library/src/__pycache__/build.cpython-314.pyc", staged)
            self._git(repo, "reset")
            with (repo / ".gitignore").open("a", encoding="utf-8") as handle:
