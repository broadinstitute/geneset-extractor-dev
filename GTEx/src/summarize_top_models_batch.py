...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--pigean_root", default=str(gtex_root() / "outputs" / "pigean_eaggl"))
    parser.add_argument("--genesets_root", default=str(gtex_root() / "outputs" / "genesets"))
    parser.add_argument("--planning_root", default=str(gtex_root() / "planning"))
...
def main() -> int:
    ...
    script_path = gtex_root() / "src" / "summarize_top_models.py"
    for tissue_id in selected_tissues:
        run_command(
            [
                ...
                str(script_path),
                "--pigean_root",
                str(Path(args.pigean_root).resolve()),
                "--genesets_root",
                str(Path(args.genesets_root).resolve()),
                "--planning_root",
