...
def main() -> int:
    ...
    allowed_models = {item.strip() for item in str(args.models).split(",") if item.strip()} if args.models else None
    ...
    for model_group in model_groups:
        ...
        if allowed_models is not None:
            summary_rows = [row for row in summary_rows if row.get("model_id") in allowed_models]
        query_rows = load_query_map(query_summary_path)
        if allowed_models is not None:
            query_rows = {
                key: value
                for key, value in query_rows.items()
                if key[0] in allowed_models
            }
