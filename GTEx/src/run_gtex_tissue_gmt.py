...
def default_tissue_model_manifest() -> Path:
    return repo_root() / "geneset-extractor-dev" / "GTEx" / "planning" / "gtex_tissue_model_step1" / "model_manifest.tsv"
...
def default_run_root(tissue_id: str) -> Path:
    return repo_root() / "geneset-extractor-dev" / "GTEx" / "outputs" / "genesets" / tissue_id / "tissue_models"
...
def build_extractor_cmd(
    *,
    ...
    if settings["ANNOTATION_MODE"] == "gtf_annotated":
        if not gtf_path:
            raise SystemExit("Models M19 and M20 require --gtf")
    ...
    *,
    ...
    text = "\n".join(
        [
            ...
            f"PYTHONPATH={shlex.quote(str(override_dir))}:{shlex.quote(str(dig_dir / 'src'))} {shell_join(extractor_cmd)}",
            "```",
            "",
        ]
    )
...
def main() -> int:
    ...
    override_dir = repo / "geneset-extractor-dev" / "GTEx" / "src"
    ...
    invocation_cmd = [str(repo / "geneset-extractor-dev" / "GTEx" / "run" / "run_gtex_tissue_gmt.sh"), *sys.argv[1:]]
    statuses: list[dict[str, Any]] = []
    env = os.environ.copy()
    env["PYTHONPATH"] = f"{override_dir}:{dig_dir / 'src'}"
