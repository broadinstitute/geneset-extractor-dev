...
meta <- read.delim("/home/ryank/software/geneset_extractors/geneset-extractor-dev/GTEx/outputs/genesets/adipose_subcutaneous/tissue_models/T2/workflow/continuous_sample_metadata.tsv", check.names=FALSE)
...
y <- calcNormFactors(y)
design <- model.matrix(as.formula("~ age_mid"), data=meta)
...
write.table(tt, file="/home/ryank/software/geneset_extractors/geneset-extractor-dev/GTEx/outputs/genesets/adipose_subcutaneous/tissue_models/T2/tissue_extractor/tissue_deg.tsv", sep="\t", row.names=FALSE, quote=FALSE)
