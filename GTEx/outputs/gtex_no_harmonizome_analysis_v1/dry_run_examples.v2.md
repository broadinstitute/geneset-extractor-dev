...
with gzip.open(counts_gct_gz_path, "rt", encoding="utf-8") as handle:
    ...
    with open(r"outputs/gtex_no_harmonizome_analysis_v1/prepared/tissue_matrices/AdiposeTissue.v1.tsv", "w", encoding="utf-8", newline="") as matrix_handle:
...
tissue_metadata_df.to_csv(r"outputs/gtex_no_harmonizome_analysis_v1/prepared/tissue_metadata/AdiposeTissue.v1.tsv", sep="\t", index=False)
...
tissue_comparisons_df.to_csv(r"outputs/gtex_no_harmonizome_analysis_v1/prepared/tissue_comparisons/AdiposeTissue.v1.tsv", sep="\t", index=False)
```
...
- explanation: Example tissue-level differential expression workflow command for the no-Harmonizome configuration. The real run repeats this pattern for each tissue in the manifest.
...
/home/ryank/software/miniconda3/envs/work/bin/python3 -m geneset_extractors.cli workflows rna_de_prepare \
  --modality bulk \
  --counts_tsv outputs/gtex_no_harmonizome_analysis_v1/prepared/tissue_matrices/AdiposeTissue.v1.tsv \
  --matrix_orientation gene_by_sample \
  --feature_id_column Name \
  --matrix_gene_symbol_column Description \
  --sample_metadata_tsv outputs/gtex_no_harmonizome_analysis_v1/prepared/tissue_metadata/AdiposeTissue.v1.tsv \
  --sample_id_column sample_id \
  --group_column age_bin \
  --comparisons_tsv outputs/gtex_no_harmonizome_analysis_v1/prepared/tissue_comparisons/AdiposeTissue.v1.tsv \
  --covariates sex,smtsd \
  --de_mode modern \
  --backend lightweight \
  --out_dir outputs/gtex_no_harmonizome_analysis_v1/rna_de_prepare/AdiposeTissue.v1 \
  --organism human \
  --genome_build hg38
```
...
- explanation: Example downstream conversion command that turns the combined DEG table into gene sets with legacy postprocessing.
- workdir: `/home/ryank/work/geneset_extractors/gtex/dig-gene-set-extractors`

```bash
/home/ryank/software/miniconda3/envs/work/bin/python3 -m geneset_extractors.cli convert rna_deg_multi \
  --deg_tsv outputs/gtex_no_harmonizome_analysis_v1/combined/deg_long_combined.v1.tsv \
  --comparison_column comparison_id \
  --out_dir outputs/gtex_no_harmonizome_analysis_v1/rna_deg_multi.v1 \
  --organism human \
  --genome_build hg38 \
