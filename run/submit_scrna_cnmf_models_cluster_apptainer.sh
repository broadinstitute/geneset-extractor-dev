...
SELF_PATH="${REPO_ROOT}/geneset-extractor-dev/run/submit_scrna_cnmf_models_cluster_apptainer.sh"
...
# Each row gives the local paths for one dataset.
...
usage() {
  cat <<'EOF'
Usage:
  submit_scrna_cnmf_models_cluster_apptainer.sh --submit [--write_model_only|--refresh_metadata_and_provenance]
      [--model_id MODEL[,MODEL...]] [--dataset_id DATASET[,DATASET...]]
  submit_scrna_cnmf_models_cluster_apptainer.sh --help
...
Optional environment variables:
  ...
  SCRNA_CONFIG_ROOT, DIG_DIR, PYTHON_BIN, QSUB_BIN
...
run_outer_worker() {
  ...
  binds="$(
    {
      ...
      append_bind_path "${SCRNA_MODEL_LIST}"
      append_bind_path "${SCRNA_MODEL_MANIFEST}"
      append_bind_path "${SCRNA_DATASET_LIST}"
      ...
      append_bind_path "${SCRNA_INPUT_MAP_TSV:-}"
    } | sort -u | grep -v '^$'
  )"
  bind_csv="$(printf '%s\n' "${binds}" | paste -sd, -)"
  ...
  env \
    ...
    APPTAINERENV_SCRNA_WORKLIST="${SCRNA_WORKLIST}" \
    APPTAINERENV_SCRNA_OUT_ROOT="${SCRNA_OUT_ROOT}" \
    APPTAINERENV_SCRNA_MODEL_LIST="${SCRNA_MODEL_LIST}" \
    APPTAINERENV_SCRNA_MODEL_MANIFEST="${SCRNA_MODEL_MANIFEST}" \
    APPTAINERENV_SCRNA_DATASET_LIST="${SCRNA_DATASET_LIST}" \
    ...
    APPTAINERENV_SCRNA_INPUT_MAP_TSV="${SCRNA_INPUT_MAP_TSV:-}" \
    "${APPTAINER_BIN}" exec \
      --bind "${bind_csv}" \
...
submit_array() {
  ...
  "${QSUB_BIN}" \
    -N "scrna_cnmf_all_models_apptainer" \
    -t "1-${task_count}" \
