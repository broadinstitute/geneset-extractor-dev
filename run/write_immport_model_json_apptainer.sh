...
usage() {
  cat <<'EOF'
Usage:
  ./geneset-extractor-dev/run/write_immport_model_json_apptainer.sh [run_immport_bulk_de_model.py args...]
  ./geneset-extractor-dev/run/write_immport_model_json_apptainer.sh --help
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --run_root|--expression_tsv|--sample_metadata_tsv|--model_manifest|--provenance_mirror_local_prefix)
...
RUNNER="${REPO_ROOT}/geneset-extractor-dev/ImmPort/src/run_immport_bulk_de_model.py"
