...
usage() {
  cat <<'EOF'
Usage:
  ./geneset-extractor-dev/run/write_geo_bulk_model_json_apptainer.sh [run_geo_bulk_model.py args...]
  ./geneset-extractor-dev/run/write_geo_bulk_model_json_apptainer.sh --help

Regenerates geneset.model.json sidecars from config inside the Apptainer image by
running GEO_BULK/src/run_geo_bulk_model.py with --write_model_only appended.
Requires APPTAINER_IMAGE. Forwards run_geo_bulk_model.py arguments unchanged.
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --dataset_list|--model_manifest|--description_templates|--dig_dir|--input_root|--out_root|--provenance_mirror_local_prefix)
...
RUNNER="${REPO_ROOT}/geneset-extractor-dev/GEO_BULK/src/run_geo_bulk_model.py"
