...
usage() {
  cat <<'EOF'
Usage:
  ./geneset-extractor-dev/run/submit_lincs_l1000_models_cluster.sh --submit [--write_model_only|--refresh_metadata_and_provenance] [--model_group HZ] [--model_id MODEL[,MODEL...]]
...
filter_refresh_existing_worklist() {
  ...
  done < <(tail -n +2 "${LINCS_WORKLIST}")
  mv "${filtered_worklist}" "${LINCS_WORKLIST}"
  if [[ ${kept} -le 0 ]]; then
    echo "No LINCS_L1000 refresh tasks selected after excluding missing outputs." >&2
    exit 1
  fi
}
