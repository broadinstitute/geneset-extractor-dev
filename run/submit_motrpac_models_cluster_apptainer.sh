...
S3_INPUT_SOURCE_MAP_TSV="${S3_INPUT_SOURCE_MAP_TSV:-}"
FILTER_MODEL_GROUP=""
FILTER_TISSUE_ID=""
FILTER_MODEL_IDS=""
...
usage() {
  cat <<'EOF'
Usage:
  ./geneset-extractor-dev/run/submit_motrpac_models_cluster_apptainer.sh --submit [--write_model_only|--refresh_metadata_and_provenance] [--model_group TR|TW|HZ] [--tissue_id TISSUE|all_tissues] [--model_id MODEL[,MODEL...]]
...
apptainer_bind_csv() {
  {
    ...
    append_bind_path "${S3_INPUT_SOURCE_MAP_TSV:-}"
...
run_task_in_apptainer() {
  ...
  APPTAINERENV_MOTRPAC_RAW_COUNTS_DIR="${MOTRPAC_RAW_COUNTS_DIR}" \
  ...
  APPTAINERENV_S3_INPUT_ROOT="${S3_INPUT_ROOT}" \
  APPTAINERENV_S3_INPUT_SOURCE_MAP_TSV="${S3_INPUT_SOURCE_MAP_TSV}" \
...
run_task() {
  ...
  if [[ ${REFRESH_METADATA_AND_PROVENANCE} -eq 1 ]]; then
    ...
    if [[ -n "${S3_INPUT_ROOT}" ]]; then
      refresh_cmd+=(--s3_input_root "${S3_INPUT_ROOT}")
    fi
    if [[ -n "${S3_INPUT_SOURCE_MAP_TSV}" ]]; then
      refresh_cmd+=(--s3_input_source_map_tsv "${S3_INPUT_SOURCE_MAP_TSV}")
    fi
    echo "+ ${refresh_cmd[*]}"
    "${refresh_cmd[@]}"
    return
