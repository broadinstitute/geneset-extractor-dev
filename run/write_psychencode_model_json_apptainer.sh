...
usage() {
  cat <<'EOF'
Usage:
  ./geneset-extractor-dev/run/write_psychencode_model_json_apptainer.sh [run_psychencode_hz_model.py args...]
  ./geneset-extractor-dev/run/write_psychencode_model_json_apptainer.sh --help
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --run_root|--input_csv|--model_manifest|--provenance_mirror_local_prefix)
...
RUNNER="${REPO_ROOT}/geneset-extractor-dev/PsychENCODE/src/run_psychencode_hz_model.py"
