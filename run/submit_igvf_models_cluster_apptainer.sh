...
SELF_PATH="${REPO_ROOT}/geneset-extractor-dev/run/submit_igvf_models_cluster_apptainer.sh"
WORK_ROOT="${WORK_ROOT:-$(pwd)}"

IGVF_CONFIG_ROOT="${IGVF_CONFIG_ROOT:-${REPO_ROOT}/geneset-extractor-dev/IGVF/config}"
DIG_DIR="${DIG_DIR:-${REPO_ROOT}/dig-gene-set-extractors}"

IGVF_MODEL_LIST="${IGVF_MODEL_LIST:-${IGVF_CONFIG_ROOT}/model_list.tsv}"
IGVF_MODEL_MANIFEST="${IGVF_MODEL_MANIFEST:-${IGVF_CONFIG_ROOT}/model_manifest.tsv}"

IGVF_OUT_ROOT="${IGVF_OUT_ROOT:-${WORK_ROOT}/igvf_all_models}"
QSUB_LOG_ROOT="${QSUB_LOG_ROOT:-${WORK_ROOT}/qsub_logs_igvf}"
IGVF_WORKLIST="${IGVF_WORKLIST:-${WORK_ROOT}/igvf_qsub_worklist.tsv}"
IGVF_EXPRESSION_TSV="${IGVF_EXPRESSION_TSV:-}"
IGVF_MAPPING_FILE="${IGVF_MAPPING_FILE:-}"
...
IGVF_ARRAY_MEMORY="${IGVF_ARRAY_MEMORY:-16G}"
IGVF_ARRAY_WALLTIME="${IGVF_ARRAY_WALLTIME:-24:00:00}"
...
usage() {
  cat <<'EOF'
Usage:
  ./geneset-extractor-dev/run/submit_igvf_models_cluster_apptainer.sh --submit [--write_model_only|--refresh_metadata_and_provenance] [--model_group PS] [--model_id MODEL[,MODEL...]]
  ./geneset-extractor-dev/run/submit_igvf_models_cluster_apptainer.sh --help

Required environment variables:
  APPTAINER_IMAGE
  IGVF_EXPRESSION_TSV          (processed per-perturbation signature matrix)

Optional environment variables:
  IGVF_MAPPING_FILE            (gene id->symbol mapping)
  WORK_ROOT
  DIG_DIR, PYTHON_BIN, QSUB_BIN
  APPTAINER_BIN, APPTAINER_EXTRA_ARGS
  APPTAINER_PYTHON_BIN
  IGVF_OUT_ROOT, QSUB_LOG_ROOT, IGVF_WORKLIST
  IGVF_ARRAY_MEMORY, IGVF_ARRAY_WALLTIME
...
resolve_model_group_for_id() {
  ...
  awk -F $'\t' -v model_id="${model_id}" '
    ...
    $model_id_col == model_id {
      if ($family_col == "perturbseq") print "PS"
      exit
    }
  ' "${IGVF_MODEL_LIST}"
}
...
validate_model_ids() {
  ...
  for requested_model_id in "${requested_model_ids[@]}"; do
    ...
    if [[ -z "${derived_group}" ]]; then
      echo "Model not found in IGVF model list: ${requested_model_id}" >&2
      exit 1
    fi
    ...
    case "$1" in
      ...
      --model_group)
        ...
        FILTER_MODEL_GROUP="$(canonicalize_model_group "$2")" || {
          echo "Unsupported IGVF model group: $2" >&2
          exit 1
        }
        shift 2
        ;;
...
prepare_common() {
  ...
  require_file "${IGVF_MODEL_LIST}"
  require_file "${IGVF_MODEL_MANIFEST}"
  if [[ ${REFRESH_METADATA_AND_PROVENANCE} -eq 1 ]]; then
    require_file "${DESCRIPTION_TEMPLATE_TSV}"
  else
    require_var IGVF_EXPRESSION_TSV
    require_file "${IGVF_EXPRESSION_TSV}"
    if [[ -n "${IGVF_MAPPING_FILE}" ]]; then
      require_file "${IGVF_MAPPING_FILE}"
    fi
  fi
}
...
write_worklist() {
  {
    ...
    awk -F $'\t' \
      ...
      $enabled_col == "true" {
        group = ""
        if ($family_col == "perturbseq") group = "PS"
        if (group == "") next
...
filter_refresh_existing_worklist() {
  ...
  head -n 1 "${IGVF_WORKLIST}" > "${filtered_worklist}"
  kept=0
  while IFS= read -r row; do
    ...
    suffix="${row#*$'\t'}"
    model_dir="${IGVF_OUT_ROOT}/genesets/all_signatures/models/${model_id}"
    if [[ -d "${model_dir}/extractor" ]]; then
      kept=$((kept + 1))
      printf "%d\t%s\n" "${kept}" "${suffix}" >> "${filtered_worklist}"
    fi
  done < <(tail -n +2 "${IGVF_WORKLIST}")
  mv "${filtered_worklist}" "${IGVF_WORKLIST}"
  if [[ ${kept} -le 0 ]]; then
    echo "No IGVF refresh tasks selected after excluding missing outputs." >&2
    exit 1
  fi
}

worklist_task_count() {
  awk 'NR > 1 { n += 1 } END { print n + 0 }' "${IGVF_WORKLIST}"
}
...
run_inner_worker() {
  ...
  IFS=$'\t' read -r _task_id model_group model_id < <(awk -F $'\t' -v task_id="${task_id}" 'NR > 1 && $1 == task_id { print $0; exit }' "${IGVF_WORKLIST}")
  if [[ -z "${model_id:-}" ]]; then
    echo "No IGVF worklist row found for task_id=${task_id}" >&2
    exit 1
  fi

  local src_root
  src_root="${REPO_ROOT}/geneset-extractor-dev/IGVF/src"
  local cmd
  if [[ ${REFRESH_METADATA_AND_PROVENANCE} -eq 1 ]]; then
    cmd=(
    ...
    cmd=(
      "${PYTHON_BIN}"
      "${src_root}/run_igvf_perturbseq_model.py"
      "--model_id" "${model_id}"
      "--run_root" "${IGVF_OUT_ROOT}/genesets/all_signatures/models"
      "--python_bin" "${PYTHON_BIN}"
      "--dig_dir" "${DIG_DIR}"
      "--expression_tsv" "${IGVF_EXPRESSION_TSV}"
      "--model_manifest" "${IGVF_MODEL_MANIFEST}"
      "--write_model_only"
    )
    if [[ -n "${IGVF_MAPPING_FILE}" ]]; then
      cmd+=("--mapping_file" "${IGVF_MAPPING_FILE}")
    fi
  else
    cmd=(
      "${PYTHON_BIN}"
      "${src_root}/build_igvf_genesets.py"
      "--models" "${model_id}"
      "--python_bin" "${PYTHON_BIN}"
      "--dig_dir" "${DIG_DIR}"
      "--model_list" "${IGVF_MODEL_LIST}"
      "--model_manifest" "${IGVF_MODEL_MANIFEST}"
      "--out_root" "${IGVF_OUT_ROOT}"
      "--overwrite"
      "--expression_tsv" "${IGVF_EXPRESSION_TSV}"
    )
    if [[ -n "${IGVF_MAPPING_FILE}" ]]; then
...
run_outer_worker() {
  ...
  binds="$(
    {
      ...
      append_bind_path "${IGVF_MODEL_LIST}"
      append_bind_path "${IGVF_MODEL_MANIFEST}"
      ...
      append_bind_path "${IGVF_EXPRESSION_TSV}"
      append_bind_path "${IGVF_MAPPING_FILE:-}"
    } | sort -u
  )"
  bind_csv="$(printf '%s\n' "${binds}" | paste -sd, -)"
  ...
  env \
    ...
    APPTAINERENV_IGVF_WORKLIST="${IGVF_WORKLIST}" \
    APPTAINERENV_IGVF_OUT_ROOT="${IGVF_OUT_ROOT}" \
    APPTAINERENV_IGVF_MODEL_LIST="${IGVF_MODEL_LIST}" \
    APPTAINERENV_IGVF_MODEL_MANIFEST="${IGVF_MODEL_MANIFEST}" \
    ...
    APPTAINERENV_IGVF_EXPRESSION_TSV="${IGVF_EXPRESSION_TSV}" \
    APPTAINERENV_IGVF_MAPPING_FILE="${IGVF_MAPPING_FILE}" \
    "${APPTAINER_BIN}" exec \
      --bind "${bind_csv}" \
...
submit_array() {
  ...
  if [[ "${task_count}" -le 0 ]]; then
    echo "No IGVF tasks selected." >&2
    exit 1
  fi

  "${QSUB_BIN}" \
    -N "igvf_all_models_apptainer" \
    -t "1-${task_count}" \
    -o "${QSUB_LOG_ROOT}/igvf.\$TASK_ID.out" \
    -e "${QSUB_LOG_ROOT}/igvf.\$TASK_ID.err" \
    -l "h_vmem=${IGVF_ARRAY_MEMORY},h_rt=${IGVF_ARRAY_WALLTIME}" \
    -v "REPO_ROOT=${REPO_ROOT},WORK_ROOT=${WORK_ROOT},IGVF_WORKLIST=${IGVF_WORKLIST},IGVF_OUT_ROOT=${IGVF_OUT_ROOT},IGVF_MODEL_LIST=${IGVF_MODEL_LIST},IGVF_MODEL_MANIFEST=${IGVF_MODEL_MANIFEST},DIG_DIR=${DIG_DIR},PYTHON_BIN=${PYTHON_BIN},APPTAINER_BIN=${APPTAINER_BIN},APPTAINER_IMAGE=${APPTAINER_IMAGE},APPTAINER_EXTRA_ARGS=${APPTAINER_EXTRA_ARGS},APPTAINER_PYTHON_BIN=${APPTAINER_PYTHON_BIN},WRITE_MODEL_ONLY=${WRITE_MODEL_ONLY},REFRESH_METADATA_AND_PROVENANCE=${REFRESH_METADATA_AND_PROVENANCE},DESCRIPTION_TEMPLATE_TSV=${DESCRIPTION_TEMPLATE_TSV},PROVENANCE_MIRROR_LOCAL_PREFIX=${PROVENANCE_MIRROR_LOCAL_PREFIX},PROVENANCE_MIRROR_REMOTE_PREFIX=${PROVENANCE_MIRROR_REMOTE_PREFIX},LOCAL_INPUT_SOURCE_MAP_TSV=${LOCAL_INPUT_SOURCE_MAP_TSV},IGVF_EXPRESSION_TSV=${IGVF_EXPRESSION_TSV},IGVF_MAPPING_FILE=${IGVF_MAPPING_FILE}" \
    "${SELF_PATH}"
}

main() {
  WORK_ROOT="$(absolute_path "${WORK_ROOT}")"
  IGVF_OUT_ROOT="$(absolute_path "${IGVF_OUT_ROOT}")"
  QSUB_LOG_ROOT="$(absolute_path "${QSUB_LOG_ROOT}")"
  IGVF_WORKLIST="$(absolute_path "${IGVF_WORKLIST}")"
