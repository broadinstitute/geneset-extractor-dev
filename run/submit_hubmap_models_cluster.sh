...
usage() {
  cat <<'EOF'
Usage:
  ./geneset-extractor-dev/run/submit_hubmap_models_cluster.sh --submit [--write_model_only|--refresh_metadata_and_provenance] [--model_group HZ] [--model_id MODEL[,MODEL...]]
...
run_worker() {
  ...
  if [[ -n "${S3_INPUT_ROOT:-}" ]]; then
    cmd+=(--s3_input_root "${S3_INPUT_ROOT}")
  fi
  if [[ -n "${S3_INPUT_SOURCE_MAP_TSV:-}" ]]; then
    cmd+=(--s3_input_source_map_tsv "${S3_INPUT_SOURCE_MAP_TSV}")
  fi
  printf '$'
  printf ' %q' "${cmd[@]}"
  printf '\n'
