#!/usr/bin/env bash
set -euo pipefail
# Submit TCGA methylation models as a qsub array (Apptainer).
...
run_task(){
  ...
  bind_csv="$( { printf '%s\n' "${REPO_ROOT}" "${DIG_DIR}" "${OUT_ROOT}"; append_bind_path "${CONFIG_ROOT}"; append_bind_path "${BETA_DIR}"; append_bind_path "${SAMPLE_SHEET_TSV}"; append_bind_path "${GTF}"; append_bind_path "${PROBE_MANIFEST_TSV}"; } | awk 'NF && !seen[$0]++' | paste -sd, - )"
  if [[ "${MODE}" == "refresh" ]]; then
    ...
    inner="'${APPTAINER_PYTHON_BIN}' '${B}/src/run_methylation_diff_model.py' --model_id '${model}' --tumor_type_id '${tt}' --tumor_type_label '${lab}' --project_id '${proj}' --run_root '${OUT_ROOT}/genesets/${tt}/models' --dig_dir '${DIG_DIR}' --model_manifest '${MODEL_MANIFEST}' --write_model_only"
  else
    inner="'${APPTAINER_PYTHON_BIN}' '${B}/src/build_tcga_meth_genesets.py' --tumor_types '${tt}' --models '${model}' --beta_dir '${BETA_DIR}' --sample_sheet_tsv '${SAMPLE_SHEET_TSV}' --gtf '${GTF}' --dig_dir '${DIG_DIR}' --out_root '${OUT_ROOT}' --model_manifest '${MODEL_MANIFEST}' --model_list '${MODEL_LIST}' --tumor_type_list '${TUMOR_TYPE_LIST}'"
