#!/usr/bin/env bash
set -euo pipefail
# Submit TCGA RNA-seq models as a qsub array (host Python).
...
JOB_NAME="${TCGA_RNASEQ_JOB_NAME:-tcga_rnaseq_models}"; QSUB_BIN="${QSUB_BIN:-qsub}"
...
run_task(){
  ...
  if [[ "${MODE}" == "write_model_only" ]]; then
    ...
    exec "${PYTHON_BIN}" "${B}/src/${r}" --model_id "${model}" --tumor_type_id "${tt}" --tumor_type_label "${lab}" \
      --project_id "${proj}" --run_root "${OUT_ROOT}/genesets/${tt}/models" --dig_dir "${DIG_DIR}" --model_manifest "${MODEL_MANIFEST}" --write_model_only
  fi
  local -a cmd
  cmd=("${PYTHON_BIN}" "${B}/src/build_tcga_rnaseq_genesets.py" --tumor_types "${tt}" --models "${model}"
       --counts_dir "${COUNTS_DIR}" --sample_sheet_tsv "${SAMPLE_SHEET_TSV}" --gtf "${GTF}"
