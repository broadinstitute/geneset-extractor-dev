#!/usr/bin/env bash
set -euo pipefail
# Submit TCGA SpliceSeq tumor-vs-normal models as a qsub array (host Python).
# One family (SP*); only projects with matched normals. Two-step: splice_prepare_public -> splice_event_matrix.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEFAULT_REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
REPO_ROOT="${REPO_ROOT:-${DEFAULT_REPO_ROOT}}"
SELF_PATH="${REPO_ROOT}/geneset-extractor-dev/run/submit_tcga_splice_models_cluster.sh"
...
JOB_NAME="${TCGA_SPLICE_JOB_NAME:-tcga_splice_models}"; QSUB_BIN="${QSUB_BIN:-qsub}"
...
run_task(){
  ...
  if [[ "${MODE}" == "write_model_only" ]]; then
    exec "${PYTHON_BIN}" "${REPO_ROOT}/geneset-extractor-dev/NCI_GDC_TCGA_Splice/src/run_splice_tumor_vs_normal_model.py" \
  ...
  local -a cmd
  cmd=("${PYTHON_BIN}" "${REPO_ROOT}/geneset-extractor-dev/NCI_GDC_TCGA_Splice/src/build_tcga_splice_genesets.py"
       --tumor_types "${tt}" --models "${model}" --psi_dir "${PSI_DIR}" --dig_dir "${DIG_DIR}"
