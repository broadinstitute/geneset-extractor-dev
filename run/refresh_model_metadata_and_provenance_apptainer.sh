...
Notes:
  ...
  - Common path-bearing arguments like --model_dir, --description_template_tsv,
    --provenance_mirror_local_prefix, and --provenance_mirror_remote_prefix are
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --model_dir|--description_template_tsv|--provenance_mirror_local_prefix|--provenance_mirror_remote_prefix)
