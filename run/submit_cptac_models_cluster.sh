...
usage() {
  cat <<'EOF'
Usage:
  ./geneset-extractor-dev/run/submit_cptac_models_cluster.sh [--submit] [--write_model_only|--refresh_metadata_and_provenance] [--cohort_id COHORT] [--model_id MODEL[,MODEL...]]
  ./geneset-extractor-dev/run/submit_cptac_models_cluster.sh --help

Required environment variables:
  (none - CPTAC/PDC CDAP reports are fetched live from the public PDC GraphQL API)
...
submit_array() {
  ...
  job_name="${CPTAC_JOB_NAME:-cptac_all_models}"
  ...
  "${QSUB_BIN}" \
    ...
    -v "REPO_ROOT=${REPO_ROOT},WORK_ROOT=${WORK_ROOT},CPTAC_WORKLIST=${CPTAC_WORKLIST},CPTAC_OUT_ROOT=${CPTAC_OUT_ROOT},CPTAC_CONFIG_ROOT=${CPTAC_CONFIG_ROOT},CPTAC_STUDY_MANIFEST=${CPTAC_STUDY_MANIFEST},CPTAC_MODEL_LIST=${CPTAC_MODEL_LIST},CPTAC_MODEL_MANIFEST=${CPTAC_MODEL_MANIFEST},DIG_DIR=${DIG_DIR},PYTHON_BIN=${PYTHON_BIN},WRITE_MODEL_ONLY=${WRITE_MODEL_ONLY},REFRESH_METADATA_AND_PROVENANCE=${REFRESH_METADATA_AND_PROVENANCE},DESCRIPTION_TEMPLATE_TSV=${DESCRIPTION_TEMPLATE_TSV},PROVENANCE_MIRROR_LOCAL_PREFIX=${PROVENANCE_MIRROR_LOCAL_PREFIX},PROVENANCE_MIRROR_REMOTE_PREFIX=${PROVENANCE_MIRROR_REMOTE_PREFIX},LOCAL_INPUT_SOURCE_MAP_TSV=${LOCAL_INPUT_SOURCE_MAP_TSV}" \
    "${BASH_SOURCE[0]}"
}
...
run_task() {
  ...
  if [[ -z "${task_id}" ]]; then
    echo "CPTAC worker requires PBS_ARRAYID or SGE_TASK_ID" >&2
    exit 1
  fi
