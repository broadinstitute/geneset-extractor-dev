...
SELF_PATH="${REPO_ROOT}/geneset-extractor-dev/run/submit_psychencode_models_cluster_apptainer.sh"
...
PREVIOUS_PROVENANCE_MIRROR_REMOTE_PREFIX="${PREVIOUS_PROVENANCE_MIRROR_REMOTE_PREFIX:-}"
...
usage() {
  cat <<'EOF'
Usage:
  ./geneset-extractor-dev/run/submit_psychencode_models_cluster_apptainer.sh --submit [--write_model_only|--refresh_metadata_and_provenance] [--model_group HZ] [--model_id MODEL[,MODEL...]]
  ./geneset-extractor-dev/run/submit_psychencode_models_cluster_apptainer.sh --help
...
Optional environment variables:
  ...
  PREVIOUS_PROVENANCE_MIRROR_REMOTE_PREFIX
...
run_inner_worker() {
  ...
  if [[ ${REFRESH_METADATA_AND_PROVENANCE} -eq 1 && -n "${PREVIOUS_PROVENANCE_MIRROR_REMOTE_PREFIX:-}" ]]; then
    cmd+=(--previous_provenance_mirror_remote_prefix "${PREVIOUS_PROVENANCE_MIRROR_REMOTE_PREFIX}")
  fi
...
run_outer_worker() {
  ...
  binds="$(
    {
      ...
      append_bind_path "${PSYCHENCODE_MODEL_LIST}"
      append_bind_path "${PSYCHENCODE_MODEL_MANIFEST}"
      ...
      append_bind_path "${PSYCHENCODE_DEX_CSV}"
      append_bind_path "${PSYCHENCODE_MODULES_CSV}"
    } | sort -u
  )"
  bind_csv="$(printf '%s\n' "${binds}" | paste -sd, -)"
  ...
  env \
    ...
    APPTAINERENV_PSYCHENCODE_WORKLIST="${PSYCHENCODE_WORKLIST}" \
    APPTAINERENV_PSYCHENCODE_OUT_ROOT="${PSYCHENCODE_OUT_ROOT}" \
    APPTAINERENV_PSYCHENCODE_MODEL_LIST="${PSYCHENCODE_MODEL_LIST}" \
    APPTAINERENV_PSYCHENCODE_MODEL_MANIFEST="${PSYCHENCODE_MODEL_MANIFEST}" \
    ...
    APPTAINERENV_PREVIOUS_PROVENANCE_MIRROR_REMOTE_PREFIX="${PREVIOUS_PROVENANCE_MIRROR_REMOTE_PREFIX}" \
    APPTAINERENV_LOCAL_INPUT_SOURCE_MAP_TSV="${LOCAL_INPUT_SOURCE_MAP_TSV}" \
    APPTAINERENV_PSYCHENCODE_DEX_CSV="${PSYCHENCODE_DEX_CSV}" \
    APPTAINERENV_PSYCHENCODE_MODULES_CSV="${PSYCHENCODE_MODULES_CSV}" \
    "${APPTAINER_BIN}" exec \
      --bind "${bind_csv}" \
...
submit_array() {
  ...
  "${QSUB_BIN}" \
    -N "psychencode_all_models_apptainer" \
    -t "1-${task_count}" \
