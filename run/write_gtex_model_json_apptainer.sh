...
usage() {
  cat <<'EOF'
Usage:
  ./geneset-extractor-dev/run/write_gtex_model_json_apptainer.sh --runner age_binned|continuous_age [runner args...]
  ./geneset-extractor-dev/run/write_gtex_model_json_apptainer.sh --help
EOF
}
...
case "${RUNNER_NAME}" in
  age_binned) RUNNER_REL="GTEx/src/run_age_binned_model.py" ;;
  continuous_age) RUNNER_REL="GTEx/src/run_continuous_age_model.py" ;;
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --run_root|--expression_gct|--sample_attributes_tsv|--subject_phenotypes_tsv|--prepared_dir|--gtf|--age_binned_model_manifest|--continuous_age_model_manifest|--provenance_mirror_local_prefix)
