...
def immport_root() -> Path:
    return repo_root() / "geneset-extractor-dev" / "ImmPort"


def config_root() -> Path:
    return immport_root() / "config"


def default_out_root() -> Path:
    return Path.cwd() / "immport_all_models"
...
def default_study_list_path() -> Path:
    return config_root() / "study_list.tsv"
