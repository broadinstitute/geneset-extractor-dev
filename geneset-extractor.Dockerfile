...
RUN Rscript -e "options(repos = c(CRAN = 'https://cloud.r-project.org')); if (!requireNamespace('rliger', quietly=TRUE)) install.packages('rliger', dependencies=FALSE)" && \
    Rscript -e "if (!requireNamespace('rliger', quietly=TRUE)) remotes::install_github('welch-lab/liger', dependencies=FALSE)" && \
    Rscript -e "pkgs <- c('rliger', 'Seurat', 'dplyr', 'purrr', 'clue', 'proxy', 'reticulate', 'anndata', 'XVector', 'SparseArray', 'DelayedArray', 'HDF5Array', 'rhdf5', 'hdf5r', 'RcppPlanc', 'leidenAlg'); missing <- pkgs[!vapply(pkgs, requireNamespace, logical(1), quietly=TRUE)]; if (length(missing)) stop(paste('Missing R packages after environment creation:', paste(missing, collapse=', ')))"
